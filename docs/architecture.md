...
graph TD
    ...
    CHK --> DB[("SQLite (SQLCipher)<br/>op-sqlite<br/>метаданные, зашифровано")]
...
| Приложение → диск | Файлы документов и метаданные заявок | Шифрование файлов + вся БД через SQLCipher, ключ в Keychain/Keystore |
...
| Сбой между удалением строки в БД и удалением файла (или между записью файла и вставкой строки) | Низкая | Зашифрованный файл документа остаётся на диске без записи в БД, пользователь его не видит | Порядок «сначала БД, потом файл»; при неудачной транзакции прикрепления записанный файл удаляется сразу. Прерывание процесса между записью и commit в Фазе 1 оставляет мусор на диске — **осознанный компромисс, не потеря данных**: уборка при запуске отложена до публикации в стор; см. [ADR-0012](./adr/0012-delete-orphan-documents-with-application.md) и CLAUDE.md |
| Расшифрованная копия выбранного файла остаётся в кэше приложения | Низкая | Скан документа открытым текстом в приватном кэше | Копия удаляется вместе с каталогом `<UUID>` сразу после чтения, при любом исходе; путь проверяется до чтения и удаления; см. `src/storage/localCopy.ts` |
