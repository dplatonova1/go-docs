 ...
 *
 * Модуль общий для `src/storage` и `src/db`: SQLite — такое же
 * хранилище, как и файлы на диске, поэтому `DatabaseFailure` живёт здесь
 * наравне с остальными. Зависимость `db` → `storage` не случайна и
 * существует и без этого файла: `client.ts` берёт у `keychain.ts` ключ
 * шифрования базы.
 *
