...
- **Цвета — только из [`../theme/colors.ts`](../theme/colors.ts),** через
  `useThemedStyles`, у каждого `Text` и у фона экрана. Умолчаний платформы
  не хватает: на Android в тёмной теме текст без цвета остаётся чёрным на
  тёмном фоне, и экран выглядит пустым.
